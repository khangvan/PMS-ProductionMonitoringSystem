...
-- ============================================= exec amevn_PMSOrderReport_byDateRange '10/21/2016', '10/22/2016'
CREATE PROCEDURE [dbo].[amevn_PMSOrderReport_byDateRange]
...
SELECT     pr.ProdOrder, pr.SAP_model, pr.Linename, COUNT(pr.ACS_Serial) AS AssemblyQty
 ,MIN(pr.Last_event_date) AS RefStartDate
 , MAX(pr.Last_event_date) 
                      AS RefEnddate, DATEDIFF(hour, MIN(pr.Last_event_date), MAX(pr.Last_event_date)) AS RefTotal
FROM         PMSRaw AS pr 
--INNER JOIN
--                      DimDate AS dd ON dd.DateKey = REPLACE(CONVERT(varchar, pr.Last_event_date, 102), '.', '') INNER JOIN
--                      DimTime AS dt ON dt.TimeKey = REPLACE(CONVERT(VARCHAR, pr.Last_event_date, 108), ':', '')
WHERE  (   

 last_event_date >=@varStart and  last_event_date<  DATEADD(dd,1, @varEnd)
)
GROUP BY pr.ProdOrder, pr.SAP_model, pr.Linename
ORDER BY RefStartDate
